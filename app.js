const CASE_LIBRARY = [
  ...
  {
    id:'CASE-002', reference:'SR-102417', title:'Locked account access', queueTitle:'Driver phone', channel:'Phone', customerType:'Driver', customer:'Jordan Lee', initials:'JL', priority:'Normal priority', wait:'Queued next', status:'queued', difficulty:'Foundational', rootCause:'Account > Maintenance > Account Lock Inquiry', category:'Account Management',
    ...
    contactLine:'Phone: (555) 010-2041', replyPlaceholder:'Document what you would tell Jordan...', xp:180, readiness:2, resultTitle:'Locked account investigated',
    resultDescription:'You documented the locked-account request and communicated the next step.',
  ...
  {
    id:'CASE-003', reference:'SR-102451', title:'Consumer report question', queueTitle:'Driver chat', channel:'Chat', customerType:'Driver', customer:'Morgan Ellis', initials:'ME', priority:'Normal priority', wait:'Queued', status:'queued', difficulty:'Foundational', rootCause:'Driver Onboarding > Consumer Report', category:'Account Management',
    ...
    contactLine:'Phone: (555) 010-3058', replyPlaceholder:'Type your response to Morgan...', xp:160, readiness:2, resultTitle:'Consumer report question addressed',
    resultDescription:'You reviewed the contact and provided a clear, documented explanation.',
    kba:{title:'Understanding consumer reports and FAQ',shortTitle:'Understanding consumer reports',url:'https://roadie.lightning.force.com/lightning/r/Knowledge__kav/ka0cv000000BvuDAAS/view'}
  },
  {
    ...
    kba:{title:'Using the initial investigation process',shortTitle:'Initial investigation process',url:'https://roadie.lightning.force.com/lightning/r/Knowledge__kav/ka0cv000000EYzVAAW/view'}
  },
  {
    id:'CASE-005', reference:'SR-102533', title:'Duplicate bank account', queueTitle:'Driver phone', channel:'Phone', customerType:'Driver', customer:'Casey Nguyen', initials:'CN', priority:'Normal priority', wait:'Queued', status:'queued', difficulty:'Intermediate', rootCause:'Payments > Duplicate Bank Account', category:'Account Management',
    ...
    contactLine:'Phone: (555) 010-4640', replyPlaceholder:'Document the resolution for Casey...', xp:220, readiness:3, resultTitle:'Duplicate bank account issue addressed',
    resultDescription:'You documented the duplicate-payment issue and communicated the approved next step.',
...
function renderLanding(){app.append(template('landing-template'));$('#startShiftBtn').onclick=()=>{resetShift();state.shiftStartedAt=Date.now();go('dashboard')}}
...
function renderCaseChecklist(){
  ...
  const directions={found:'Use Global Search with the customer phone number.',reviewed:'Open all three profiles from the search results.',trustSafetyChecked:'Open Compare Profiles and complete the Trust & Safety check.',identityVerified:'In Compare Profiles, compare the legal name, date of birth, and address.',primary:'Select the correct primary account from Compare Profiles.',unlocked:'Open the selected primary profile and choose Unlock Primary Account.',loginConfirmed:'Open Customer response and draft a message to Avery asking whether they can log in.',cleared:'Open each duplicate profile, select Driving Info, and remove the license information.',noted:'Open each profile and enter the required note from the Knowledge Base Article (KBA) on handling duplicate profiles.',caseDetails:'Open Case details and save the required fields.',caseNotes:'Open Case notes and save complete documentation.',responded:'Open Customer response and send your message.'};
...
function renderCaseTab(){
  ...
  if(state.caseTab==='reply'){
    ...
    $('#saveResponseBtn').onclick=()=>{const text=$('#chatReply').value.trim();if(!text)return;state.customerResponse=text;record('SAVE_CUSTOMER_RESPONSE_DRAFT');mountCasePanel()};
    $('#sendAccessMessageBtn').onclick=()=>{const text=$('#chatReply').value.trim();if(text.length<20){alert('Draft a complete response before sending.');return}state.customerResponse=text;state.messages=[{text}];state.accessMessageSent=true;state.progress.responded=true;record('CUSTOMER_RESPONSE_SENT');mountCasePanel()};
    const receiveReply=$('#simulateLoginReplyBtn');if(receiveReply)receiveReply.onclick=()=>{state.loginConfirmed=true;state.progress.loginConfirmed=true;record('LOGIN_CONFIRMED_BY_CUSTOMER');mountCasePanel()};
  }
  if(state.caseTab==='details'){
    ...
    mount.innerHTML=`<div class="case-tab-pane"><div class="field-grid"><div><label>Customer type</label><input value="${escapeHtml(c.customerType)}" disabled></div><div><label>Channel</label><input value="${escapeHtml(c.channel)}" disabled></div></div>${duplicateChecks}<label>Category</label><select id="caseCategory"><option value="">Select category</option><option>Account Management</option><option>Gig Support</option></select><label>Root cause</label><input id="caseRoot" value="${escapeHtml(state.caseFields.rootCause)}" placeholder="Enter the root cause"><label>Gig ID</label><input id="caseGig" value="${escapeHtml(state.caseFields.gigId)}" placeholder="Not applicable"><button class="secondary full" id="saveCaseDetailsBtn">Save Case Details</button></div>`;
    $('#caseCategory').value=state.caseFields.category;
    if(c.detailedAdmin){$('#trustSafetyReview').value=state.caseFields.trustSafetyReview||'';$('#identityReview').value=state.caseFields.identityReview||'';$('#trustSafetyReview').disabled=Boolean(state.progress.trustSafetyChecked);$('#identityReview').disabled=Boolean(state.progress.identityVerified)}
    $('#saveCaseDetailsBtn').onclick=()=>{state.caseFields={category:$('#caseCategory').value,rootCause:$('#caseRoot').value.trim(),gigId:$('#caseGig').value.trim(),trustSafetyReview:c.detailedAdmin?$('#trustSafetyReview').value:'',identityReview:c.detailedAdmin?$('#identityReview').value:''};if(c.detailedAdmin){state.progress.trustSafetyChecked=state.caseFields.trustSafetyReview==='clear';state.progress.identityVerified=state.caseFields.identityReview==='match'}state.progress.caseDetails=state.caseFields.category===c.category&&state.caseFields.rootCause.trim().toLowerCase()===c.rootCause.toLowerCase()&&(!c.detailedAdmin||(state.progress.trustSafetyChecked&&state.progress.identityVerified));record('SAVE_CASE_DETAILS');mountCasePanel()};
  }
  if(state.caseTab==='notes'){
    const required=c.detailedAdmin?['found','reviewed','trustSafetyChecked','identityVerified','primary','unlocked','loginConfirmed','cleared','noted','responded','caseDetails','caseNotes']:['responded','caseDetails','caseNotes'];
