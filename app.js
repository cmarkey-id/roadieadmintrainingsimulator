...
function renderReplyPane(mount){
  ...
  $('#sendAccessMessageBtn').onclick=()=>{const text=$('#chatReply').value.trim();if(text.length<20){alert('Draft a complete response before sending.');return}state.customerResponse=text;state.messages=[{text}];state.accessMessageSent=true;state.progress.responded=true;record('CUSTOMER_RESPONSE_SENT');refreshCasePanelContent()};
...
function renderDetailsPane(mount){
  ...
  $('#saveCaseDetailsBtn').onclick=()=>{
    ...
    state.progress.caseDetails=categoryCorrect&&rootCauseCorrect&&reviewsComplete;
    ...
    feedback.textContent=!categoryCorrect?'Select the category that best matches this contact.':!rootCauseCorrect?'Review the customer’s reason for contact and select the matching approved root cause.':'Complete the Trust & Safety and identity review fields before saving.';
  };
}
...
function renderNotesPane(mount){
  ...
  $('#saveCaseNotesBtn').onclick=()=>{state.caseNotes=$('#caseNotes').value.trim();state.progress.caseNotes=state.caseNotes.length>=30;record('SAVE_CASE_NOTES');refreshCasePanelContent()};
