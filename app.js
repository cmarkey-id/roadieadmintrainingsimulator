...
const LEARNER_PROGRESS_KEY='roadie-training-learner-progress-v1';
function defaultPracticeRecord(){return{completed:false,completions:0,bestXp:0,lastCompletedAt:null}}
function loadPracticeHistory(){
  let saved={};
  try{saved=JSON.parse(localStorage.getItem(PRACTICE_HISTORY_KEY))||{}}catch{saved={}}
  const merged={...saved};
  CASE_LIBRARY.forEach(({id})=>{
    const existing=merged[id];
    merged[id]=existing&&typeof existing==='object'
      ?{...defaultPracticeRecord(),...existing}
      :defaultPracticeRecord();
  });
  localStorage.setItem(PRACTICE_HISTORY_KEY,JSON.stringify(merged));
  return merged;
}
function loadLearnerProgress(){try{const saved=JSON.parse(localStorage.getItem(LEARNER_PROGRESS_KEY))||{};return{xp:Number(saved.xp)||0,readiness:Math.min(100,Number(saved.readiness)||0),streak:Number(saved.streak)||0}}catch{return{xp:0,readiness:0,streak:0}}}
const savedLearnerProgress=loadLearnerProgress();
let state={route:'landing',practiceMode:'shift',libraryFilters:{category:'all',difficulty:'all'},practiceHistory:loadPracticeHistory(),currentCaseIndex:0,caseStatuses:CASE_LIBRARY.map((_,i)=>i===0?'available':'queued'),activeProfileId:null,query:'',startedAt:null,caseAccepted:false,casePanelOpen:true,caseTab:'reply',messages:[],customerResponse:'',caseNotes:'',caseFields:{category:'',rootCause:'',gigId:'',trustSafetyReview:'',identityReview:''},loginConfirmed:false,accessMessageSent:false,hintsUsed:0,coachHintIndex:0,coachHintText:'',coachTipUnread:false,reviewedIds:[],actions:[],progress:{},workflowFeedback:'',casesCompleted:0,xp:savedLearnerProgress.xp,readiness:savedLearnerProgress.readiness,streak:savedLearnerProgress.streak,lastCompletedCaseIndex:null,shiftStartedAt:null,kbaReferencedAt:null,kbaReferenceCount:0};
...
function resetShift(){state.currentCaseIndex=0;state.caseStatuses=CASE_LIBRARY.map((_,i)=>i===0?'available':'queued');state.casesCompleted=0;state.lastCompletedCaseIndex=null;state.shiftStartedAt=null;resetCurrentCase();}
function savePracticeHistory(){localStorage.setItem(PRACTICE_HISTORY_KEY,JSON.stringify(state.practiceHistory))}
function saveLearnerProgress(){localStorage.setItem(LEARNER_PROGRESS_KEY,JSON.stringify({xp:state.xp,readiness:state.readiness,streak:state.streak}))}
function resetSavedProgress(){
  const confirmed=window.confirm('Reset all saved training progress? Completed cases, XP, Readiness, and Streak will return to their starting values. This cannot be undone.');
  if(!confirmed)return;
...
function updateHeader(){const readiness=document.getElementById('headerReadiness');const xp=document.getElementById('headerXp');const streak=document.getElementById('headerStreak');const previousXp=Number(xp?.dataset.value||0);const previousReadiness=Number(readiness?.dataset.value||0);const previousStreak=Number(streak?.dataset.value||0);if(readiness){readiness.textContent=`${state.readiness}%`;readiness.dataset.value=state.readiness}if(xp){xp.textContent=state.xp;xp.dataset.value=state.xp}if(streak){streak.textContent=state.streak;streak.dataset.value=state.streak}if(state.xp>previousXp)animateMetric(xp);if(state.readiness>previousReadiness)animateMetric(readiness);if(state.streak>previousStreak)animateMetric(streak);const readinessProgress=document.getElementById('headerReadinessProgress');if(readinessProgress)readinessProgress.style.width=`${state.readiness}%`;const level=document.getElementById('headerLevel');if(level)level.textContent=state.xp>=XP_LEVEL_TARGET?'Skilled Specialist':'Developing Specialist';const progressText=document.getElementById('headerXpProgressText');if(progressText)progressText.textContent=state.xp>=XP_LEVEL_TARGET?'Skilled Specialist achieved':`${state.xp.toLocaleString()} / ${XP_LEVEL_TARGET.toLocaleString()} XP to Skilled Specialist`;updateShiftTimer();document.getElementById('currentProfileNav').disabled=!state.activeProfileId;updateCoachToggle();}
function updateCoachToggle(){const button=document.getElementById('coachToggle');const badge=document.getElementById('coachTipBadge');if(!button)return;button.disabled=false;button.classList.toggle('is-unlocked',state.caseAccepted);button.setAttribute('aria-label','Open Training Coach');button.title=state.caseAccepted?'Open Coach for case-specific guidance':'Open Training Coach';if(badge)badge.hidden=!state.coachTipUnread;}
...
function renderLibrary(){
  ...
  $('#resetProgressBtn').onclick=resetSavedProgress;
...
function renderDashboard(){
  ...
  $('#acceptContactBtn').onclick=()=>{state.caseAccepted=true;state.coachTipUnread=true;state.caseStatuses[state.currentCaseIndex]='active';state.startedAt=state.startedAt||Date.now();record('CONTACT_ACCEPTED',c.id);state.casePanelOpen=true;go(c.detailedAdmin?'search':'dashboard')};
...
function renderCaseChecklist(){
  ...
  const directions={found:'Enter the customer’s full phone number in Search.',reviewed:'Open all three profiles from the search results.',trustSafetyChecked:'Open Compare Profiles and complete the Trust & Safety check.',identityVerified:'In Compare Profiles, compare the legal name, date of birth, and address.',primary:'Select the correct primary account from Compare Profiles.',unlocked:'Open the selected primary profile and choose Unlock Primary Account.',loginConfirmed:'Open Customer response and draft a message asking the driver to confirm login.',cleared:'Open each duplicate profile, select Driving Info, and remove the license information.',noted:'Open each profile and enter the required note from the assigned KBA.',caseDetails:'Open Case details and save the required fields.',caseNotes:'Open Case notes and save complete documentation.',responded:'Open Customer response and send your message.'};
...
function nextCoachAction(){
  const c=currentCase();
  if(state.route==='landing')return{status:'Practice not started',action:'Choose a Practice Library scenario or begin the Full Shift Simulation.'};
...
function coachHintsForCase(c){
  if(c.detailedAdmin)return hints;
  const byWorkflow={
    locked:['Review the profile notes before changing the account status.','Confirm the Consumer Report is clear and the lock is not tied to research, discipline, or a restricted region.','After unlocking the profile, reset the password and ask the driver to confirm they can log in.'],
    consumer:['Start by opening the driver profile and confirming the preferred address.','The preferred address determines whether the Washington or Colorado transfer rule applies.','Review the Consumer Report status, then use the approved In Progress explanation before transferring to Trust & Safety.'],
    address:['Open the Gig and confirm whether a driver is already assigned.','Check the sender type, compliance location, and corrected distance before choosing an address path.','For this six-mile correction, request POD, complete the manual delivery, and apply the approved $6 mileage credit.'],
    bank:['Review the profile notes for the payment-service error before searching elsewhere.','Protect privacy: do not search for or disclose the linked account.','Use the Duplicate Bank Account macro and direct the driver to add a different bank account or debit card.']
  };
  return byWorkflow[c.workflow]||['Review the customer’s message, then identify the first record or policy you need to verify.'];
}
function openCoach(){
  const d=$('#coachDialog');
  const guidance=state.caseAccepted?nextCoachAction():{status:'Ready when you are',action:'Choose a Practice Library scenario or begin the Full Shift Simulation. After you accept a contact, I’ll help you think through the next step.'};
  const practiceMode=state.practiceMode==='library';
  state.coachTipUnread=false;
  updateCoachToggle();
  $('#coachTitle').textContent=state.caseAccepted?'Waffles is ready to help':'Meet Waffles';
  $('#coachIntro').textContent=state.caseAccepted?(practiceMode?'I’ll give you Practice support for this scenario. You’ll still make the decisions and work the case yourself.':'I’ll give you Shift support while you work through today’s contacts. You’ll still make the decisions and work each case yourself.'):'Hi! I’m Waffles, your Training Coach. I can explain how practice works now and offer case-specific guidance after you accept a contact.';
  $('#coachModeLabel').textContent=state.caseAccepted?(practiceMode?'PRACTICE SUPPORT':'SHIFT SUPPORT'):'TRAINING COACH';
  $('#coachStatus').textContent=guidance.status;
  $('#coachNextAction').textContent=guidance.action;
  $('#coachHintOutput').textContent=state.coachHintText;
  $('#coachHintOutput').hidden=!state.coachHintText;
  const libraryCompleted=CASE_LIBRARY.filter(item=>state.practiceHistory[item.id]?.completed).length;
  $('#coachCases').textContent=`${practiceMode?libraryCompleted:state.casesCompleted} / ${CASE_LIBRARY.length}`;
  $('#coachHints').textContent=state.hintsUsed;
  $('#coachStreak').textContent=state.streak;
  $('#coachHintBtn').onclick=()=>{
    if(!state.caseAccepted){$('#coachHintOutput').textContent='Start by choosing a Practice Library scenario or beginning the Full Shift Simulation. Accept a contact when you’re ready to work the case.';$('#coachHintOutput').hidden=false;return}
    const available=coachHintsForCase(currentCase());
    const hint=available[Math.min(state.coachHintIndex,available.length-1)];
    state.coachHintIndex=Math.min(state.coachHintIndex+1,available.length-1);
    state.coachHintText=hint;
    state.hintsUsed+=1;
    $('#coachHints').textContent=state.hintsUsed;
    $('#coachHintOutput').textContent=hint;
    $('#coachHintOutput').hidden=false;
    record('COACH_HINT_USED',currentCase().id);
  };
  d.showModal();
}
function renderResults(){app.append(template('results-template'));const c=CASE_LIBRARY[state.lastCompletedCaseIndex??state.currentCaseIndex];const elapsed=Math.max(1,Math.round((Date.now()-state.startedAt)/60000));$('#resultTitle').textContent=c.resultTitle;$('#resultDescription').textContent=c.resultDescription;$('#resultOutcome').textContent='Resolved';$('#resultXp').textContent=`+${c.xp}`;$('#resultXp').classList.add('xp-reward');$('#resultReadiness').textContent=`+${c.readiness}%`;$('#resultTime').textContent=`${elapsed} min`;const wrongPrimary=state.actions.some(a=>a.type==='MARK_PRIMARY'&&a.targetId!=='TRN-ACC-001');const wrongUnlock=state.actions.some(a=>a.type==='UNLOCK_ACCOUNT'&&a.targetId!=='TRN-ACC-001');$('#processRating').textContent=wrongPrimary||wrongUnlock?'Recovered':'Strong';const referencedKba=state.actions.some(a=>a.type==='OPEN_KNOWLEDGE_BASE');const kbaCoaching=referencedKba?` You referenced the assigned Knowledge Base Article before completing the case${state.kbaReferenceCount>1?` (${state.kbaReferenceCount} opens)`:''}.`:' You completed the case without opening the assigned Knowledge Base Article. Use the current KBA whenever you need to verify a procedure or required language.';const detailedCopy=c.detailedAdmin?(wrongPrimary||wrongUnlock?'You ultimately resolved the case, but made an account change before confirming the correct primary profile. Compare consumer report status and account activity before taking action.':'You checked Trust & Safety status, verified matching identity details, applied the primary-account hierarchy, kept the duplicate accounts locked, removed their license information, used the required notes, and confirmed the driver could log in.'):scenarioCoaching(c);$('#coachingSummary').textContent=detailedCopy+kbaCoaching;const libraryMode=state.practiceMode==='library';const nextScenario=CASE_LIBRARY.find(item=>item.id!==c.id&&!state.practiceHistory[item.id]?.completed);$('#wafflesCelebrationTitle').textContent=state.xp>=XP_LEVEL_TARGET?'You reached Skilled Specialist!':'Nice work!';$('#wafflesCelebrationText').textContent=libraryMode?(nextScenario?`You completed ${c.title}. Try ${nextScenario.title} next to practice a different workflow.`:`You’ve completed every Practice Library scenario. Repeat any case you want to strengthen.`):(state.casesCompleted===CASE_LIBRARY.length?'You completed the full shift. Review your feedback, then view the shift summary.':'One contact down. Review your coaching feedback, then continue when you’re ready.');$('#continueShiftBtn').textContent=libraryMode?'Return to Practice Library':state.casesCompleted===CASE_LIBRARY.length?'View Shift Summary':'Continue Shift';$('#continueShiftBtn').onclick=()=>libraryMode?go('library'):advanceShift();$('#repeatCaseBtn').onclick=()=>{const idx=state.lastCompletedCaseIndex;state.caseStatuses[idx]='available';state.currentCaseIndex=idx;state.casesCompleted=Math.max(0,state.casesCompleted-1);resetCurrentCase();go('dashboard')}}
